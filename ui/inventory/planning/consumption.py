...
def render_black_white_color_summary(
    ...
    if not category:
        materials = sorted({
            str(value).strip()
            for value in black_white_df.get(
        ...
        if materials:
            summary_filter_title = (
                f"{summary_filter_title} · {'/'.join(materials)}"
            )
    summary_title = (
        f"{summary_filter_title} {t('库存汇总（同材质合并品牌）')}"
        if summary_filter_title
        else t("黑白短袖库存汇总（同材质合并品牌）")
    )
