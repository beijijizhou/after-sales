...
TEXT = {
    "zh": {
        "title": "每日出库",
        "notice": "人工登记是正式每日出库：可直接在表格修改 SKU 数量，确认后扣减库存；系统读取只作参考和备用。",
        "download": "下载仓库每日出货模板",
        ...
        "inventory_issue_help": "系统已停止整批保存，其他正常行不会被遗漏。库存不足可在下方生成临时库存调整后继续出库。",
        "missing_sku_help": "其中 {count} 个 SKU 不存在，不能用临时调整创建，请先到 SKU 管理新增。",
        "temporary_title": "用临时库存调整补足缺口",
        "temporary_help": "系统只补足已存在 SKU 的库存缺口，并生成独立流水。补足后会保留当前出库内容，供你继续确认；之后可通过库存流水和撤销功能核对修正。",
        "temporary_quantity": "临时补足数量",
        "temporary_confirm": "补足缺口并继续本批出库",
        "temporary_failed": "临时库存调整失败",
        "temporary_saved": "已临时补足 {count} 个 SKU、共 {quantity:,} 件；请继续核对并确认本批出库。",
    ...
    "en": {
        ...
        "batch_date": "Outbound batch date",
        ...
        "inventory_issue_help": "The entire batch has been stopped so valid rows are not omitted. Existing SKUs with insufficient stock can be temporarily replenished below.",
        "missing_sku_help": "{count} SKU(s) do not exist and cannot be created by a temporary adjustment. Add them in SKU Management first.",
        "temporary_title": "Fill shortages with a temporary adjustment",
        "temporary_help": "Only shortages for existing SKUs are added, in a separate auditable batch. Your outbound entries remain available for confirmation and can be corrected later through the ledger and reversal tools.",
        "temporary_quantity": "Temporary quantity",
        "temporary_confirm": "Fill shortages and continue this outbound",
        "temporary_failed": "Temporary inventory adjustment failed",
        "temporary_saved": "Temporarily added {quantity:,} pieces across {count} SKU(s). Review and confirm this outbound.",
    ...
    "es": {
        ...
        "batch_date": "Fecha del lote de salida",
        ...
        "inventory_issue_help": "Se detuvo todo el lote para no omitir filas válidas. Puede reponer temporalmente abajo los SKU existentes sin inventario suficiente.",
        "missing_sku_help": "{count} SKU no existen y no se pueden crear con un ajuste temporal. Agréguelos primero en Administración de SKU.",
        "temporary_title": "Cubrir faltantes con un ajuste temporal",
        "temporary_help": "Solo se agregan los faltantes de SKU existentes en un lote auditable separado. La salida actual se conserva para confirmarla y corregirla después desde el historial y la reversión.",
        "temporary_quantity": "Cantidad temporal",
        "temporary_confirm": "Cubrir faltantes y continuar esta salida",
        "temporary_failed": "Falló el ajuste temporal de inventario",
        "temporary_saved": "Se agregaron temporalmente {quantity:,} unidades en {count} SKU. Revise y confirme esta salida.",
