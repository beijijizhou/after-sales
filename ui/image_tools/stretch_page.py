...
def render_middle_stretch():
    uploaded_file = st.file_uploader(
        "上传图片",
        type=["png", "jpg", "jpeg", "webp"],
        help="支持 PNG、JPG、JPEG 和 WebP",
        key="middle_stretch_image",
    )
    ...
    try:
        result = stretch_image_middle(
            original, stretch_start, stretch_end, stretch_factor
        )
    ...
    with original_col:
        ...
        st.image(
            build_stretch_region_preview(
                original, stretch_start, stretch_end
            ),
            width="stretch",
        )
        st.caption(
    ...
    st.download_button(
        ...
        file_name=f"{Path(uploaded_file.name).stem}_中段拉伸.png",
        mime="image/png",
        width="stretch",
    )
