...
| Colored daily ERP audit | `ui/inventory/planning/colored_daily_audit.py`, `db/production_consumption.py`, `automation/sync/colored_models.py` | Read the shared ERP daily fact and active system-deduction ledger independently. Production-fact and coverage queries must page through every Supabase row instead of accepting the 1,000-row response default. Show all dates in the model window, platform coverage, ERP quantity, deduction quantity and difference; selecting a date opens a platform/color wide table in fixed size order. Never reconstruct ERP production from inventory deductions. |
